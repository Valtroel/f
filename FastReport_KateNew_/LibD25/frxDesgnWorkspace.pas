...
var
  i, j: Integer;
  sl: {$IFDEF NONWINFPC}TfrxStringList{$ELSE}TStringList{$ENDIF};
  b: TfrxBand;
    ...
    if AttachObjects then
      ...
      begin
        ...
        if (c is TfrxView) and (c.AbsTop >= Bnd.Top - 1e-4) and (c.AbsTop < Bnd.Top + Bnd.Height + 1e-4) then
          if ((c.Left < (Bnd.left + Bnd.Width - 1e-4)) and (TfrxReportPage(Bnd.Page).Columns > 1)) or (TfrxReportPage(Bnd.Page).Columns <= 1) then
...
begin
  sl := {$IFDEF NONWINFPC}TfrxStringList.Create{$ELSE}TStringList.Create{$ENDIF};
  sl.Sorted := True;
...
{$ENDIF}
  ...
  if SelectedCount = 0 then
  begin
    NotifyRect := frxRect(X / FScale, Y / FScale, 0, 0);
    fl := True;
  end
  else
    NotifyRect := GetSelectionBounds;
