...
uses
  ...
  frxClass, Variants
  {$IFDEF FPC}
  , Types, LCLType, LCLIntf, LCLProc,LazUTF8, LazHelper, Printers
  {$ELSE}
  , frxUnicodeUtils
  {$ENDIF}
{$IFDEF Delphi10}
...
var
  ...
  begin
    Result := False;
    if {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF} >= 2 then
      for i := 1 to {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF} do
        if {$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}(
          {$IFDEF FPC}UTF8Copy(s,i,1){$ELSE}s[i]{$ENDIF}, glasn) <> 0 then
        begin
          Result := True;
          break;
        end;
...
begin
  Result := '';
  if {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF} < 4 then
    Exit;
  ...
  begin
    CanBreak := False;
    if {$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}(
       {$IFDEF FPC}UTF8Copy(s,i,1){$ELSE}s[i]{$ENDIF}, soglasn) <> 0 then
    begin
      CanBreak := True;
      { 2 }
      if (i < {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF}) and
        ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}(
          {$IFDEF FPC}UTF8Copy(s, i + 1, 1){$ELSE}s[i + 1]{$ENDIF}, glasn) <> 0) then
        CanBreak := False;
      { 3 }
      if (i < {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF}) and
        ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}(
        {$IFDEF FPC}UTF8Copy(s, i + 1, 1){$ELSE}s[i + 1]{$ENDIF}, znaks) <> 0) then
        CanBreak := False;
    end;
    if {$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}(
      {$IFDEF FPC}UTF8Copy(s,i,1){$ELSE}s[i]{$ENDIF}, glasn) <> 0 then
    begin
      ...
      if (i < {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF}) and
        ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}(
        {$IFDEF FPC}UTF8Copy(s, i + 1, 1){$ELSE}s[i + 1]{$ENDIF}, znaks1) <> 0) then
        CanBreak := False;
      { 6 }
      if (i < {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF} - 2) and
        ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}(
         {$IFDEF FPC}UTF8Copy(s, i + 1, 1){$ELSE}s[i + 1]{$ENDIF}, soglasn) <> 0) and
        ({$IFDEF FPC}UTF8Copy(s, i + 1, 1){$ELSE}s[i + 1]{$ENDIF} =
         {$IFDEF FPC}UTF8Copy(s, i + 2, 1){$ELSE}s[i + 2]{$ENDIF}) and
          ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}(
           {$IFDEF FPC}UTF8Copy(s, i + 3, 1){$ELSE}s[i + 3]{$ENDIF}, glasn) <> 0) then
        CanBreak := False;
    end;
    if CanBreak then
      Result := Result + Chr(i);
  ...
  begin
    j := Ord(Result[i]);
    if not (Check1and5({$IFDEF FPC}UTF8Copy(s, 1, j){$ELSE}Copy(s, 1, j){$ENDIF}) and
       Check1and5({$IFDEF FPC}UTF8Copy(s, j + 1, 255){$ELSE}Copy(s, j + 1, 255){$ENDIF})) then
      Result[i] := #255;
  end;
...
begin
  i := 1;
  if {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(s) = 0 then
    Exit;

  while i <= {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(s) do
  begin
    b := True;

    if FAllowTags then
      if {$IFDEF FPC}UTF8Copy(s, i, 1){$ELSE}s[i]{$ENDIF} = '<' then
      begin

        // <b>, <u>, <i> tags
        if (i + 2 <= {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(s)) and
          ({$IFDEF FPC}UTF8Copy(s,i + 2, 1){$ELSE}s[i + 2]{$ENDIF} = '>') then
        begin
          {$IFDEF FPC}
            fps := UTF8UpperCase(UTF8Copy(s, i + 1, 1));
            if fps = 'B' then
        ...
        else if (i + 1 <= {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(s)) and
            ({$IFDEF FPC}UTF8Copy(s,i + 1, 1){$ELSE}s[i + 1]{$ENDIF} = '/') then
        begin
          if (i + 3 <= {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(s)) and
             ({$IFDEF FPC}UTF8Copy(s,i + 3, 1){$ELSE}s[i + 3]{$ENDIF} = '>') then
          begin
            {$IFDEF FPC}
              fps := UTF8UpperCase(UTF8Copy(s, i + 2, 1));
              if fps = 'B' then
                FStyle := FStyle - [fsBold]
              else if fps = 'I' then
                FStyle := FStyle - [fsItalic]
          ...
          else if ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('STRIKE>',
             {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 2) then
          begin
          ...
          else if ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('NOWRAP>',
            {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 2) then
          begin
            FDontWRAP := False;
            {$IFDEF FPC}
          ...
          else if {$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('FONT>',
            {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 2 then
          begin
            FColor := FDefColor;
            {$IFDEF FPC}
          ...
          else if ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('SUB>',
               {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 2) or
            ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('SUP>',
              {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 2) then
          begin
            FSize := FDefSize;
            FAddY := 0;
        ...
        // <sub>, <sup> tags
        else if (i + 4 <= {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(s))
          and ({$IFDEF FPC}UTF8Copy(s,i + 4, 1){$ELSE}s[i + 4]{$ENDIF} = '>') then
        begin
          b := False;
          if {$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('SUB>',
            {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 1 then
          begin
          ...
          else if {$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('SUP>',
            {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 1 then
          begin
            FSize := Round(FDefSize / 1.5);
            FAddY := 0;
        ...
        // <sub>, <sup> tags
        else if (i + 5 <= {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(s)) and
           ({$IFDEF FPC}UTF8Copy(s,i + 5, 1){$ELSE}s[i + 5]{$ENDIF} = '>') then
        begin
          b := False;
          if ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('/SUB>',
            {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 1) or
            ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('/SUP>',
            {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 1) then
          begin
            FSize := FDefSize;
        ...
        else if (i + 7 <= {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(s)) and
            (({$IFDEF FPC}UTF8Copy(s,i + 1, 1){$ELSE}s[i + 1]{$ENDIF} = 'n') or
            ({$IFDEF FPC}UTF8Copy(s,i + 1, 1){$ELSE}s[i + 1]{$ENDIF} = 'N')) then
        begin
          if {$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('NOWRAP>',
             {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 1 then
          begin
            FDontWRAP := True;
            {$IFDEF FPC}
        ...
        // <strike> tag
          ...
          (({$IFDEF FPC}UTF8Copy(s,i + 1, 1){$ELSE}s[i + 1]{$ENDIF} = 's') or
           ({$IFDEF FPC}UTF8Copy(s,i + 1, 1){$ELSE}s[i + 1]{$ENDIF} = 'S')) then
        begin
          if {$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('STRIKE>',
            {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 1 then
          begin
            FStyle := FStyle + [fsStrikeOut];
            {$IFDEF FPC}
  ...
  // <font color = ...> tag
        else if {$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}('FONT COLOR',
          {$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(s)) = i + 1 then
        begin
          j := i + 11;
          while (j <= {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(s))
             and ({$IFDEF FPC}UTF8Copy(s,j, 1){$ELSE}s[j]{$ENDIF} <> '=') do
            Inc(j);
          Inc(j);
          while (j <= {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(s))
             and ({$IFDEF FPC}UTF8Copy(s,j, 1){$ELSE}s[j]{$ENDIF} = ' ') do
          ...
          cl := {$IFDEF FPC}UTF8Copy{$ELSE}Copy{$ENDIF}(s, j1, j - j1);
          if cl <> '' then
          begin
            if ({$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(cl) > 3)
...
begin
  ...
  {$IFDEF FPC}
  n := UTF8Length(s);//Length(UTF8ToUTF16(s));
  {$ENDIF}
...
var
  ...
  begin
    ...
    i := CurPos;
    while (i <= LineEnd) and
     ({$IFDEF FPC}UTF8Pos{$ELSE}Pos{$ENDIF}({$IFDEF FPC}UTF8Copy(s, i, 1){$ELSE}s[i]{$ENDIF}, ' .,-;') = 0) do
      Inc(i);
    TheWord := {$IFDEF FPC}UTF8Copy{$ELSE}Copy{$ENDIF}(s, LineBegin, i - LineBegin);
    // get available break positions
    Breaks := BreakRussianWord({$IFDEF FPC}UTF8UpperCase{$ELSE}AnsiUpperCase{$ENDIF}(TheWord));
    ...
    if BreakPos <> CurPos - LineBegin then
      Result := {$IFDEF FPC}UTF8Copy{$ELSE}Copy{$ENDIF}(TheWord, 1, BreakPos) else
      Result := '';
  end;
...
// remove all HTML tags and build the tag list
  ...
  n := {$IFDEF FPC}UTF8Length(s){$ELSE}Length(s){$ENDIF};
...
// text fits, no need to wrap it
  ...
  begin
    ...
    //dbgv := FTempArray^[i - 1{$IFDEF FPC} - addI{$ENDIF}];
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if WasBreak then
              ...
              FText.Add(TheWord);
            BreakPos := {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(TheWord);
            FHTMLTags.Wrap(BreakPos, WasBreak);
            LastSpace := LineBegin + BreakPos - 1;
          end
          else
          begin
            //s1 := {$IFDEF FPC}UTF8Copy{$ELSE}Copy{$ENDIF}(s, LineBegin, i - LineBegin);
            FText.Add({$IFDEF FPC}UTF8Copy{$ELSE}Copy{$ENDIF}(s, LineBegin, i - LineBegin));
        ...
        begin
          //s1 := {$IFDEF FPC}UTF8Copy(s, LineBegin, 1){$ELSE}s[LineBegin]{$ENDIF};
          FText.Add({$IFDEF FPC}UTF8Copy(s, LineBegin, 1){$ELSE}s[LineBegin]{$ENDIF}); // can't wrap 1 symbol, just add it to the new line
      ...
      begin
        ...
        begin
          ...
          if WasBreak then
          begin
            //s2 :=  {$IFDEF FPC}UTF8Copy{$ELSE}Copy{$ENDIF}(s, LineBegin, LastSpace - LineBegin + 1) + TheWord + '-';
            FText.Add({$IFDEF FPC}UTF8Copy{$ELSE}Copy{$ENDIF}(s, LineBegin, LastSpace - LineBegin + 1) + TheWord + '-')
          end
          else
          begin
            //s2 :=  {$IFDEF FPC}UTF8Copy{$ELSE}Copy{$ENDIF}(s, LineBegin, LastSpace - LineBegin);
            FText.Add({$IFDEF FPC}UTF8Copy{$ELSE}Copy{$ENDIF}(s, LineBegin, LastSpace - LineBegin));
          end;
  ...
  if n - LineBegin + 1 > 0 then   // put the rest of line to FText
    FText.Add({$IFDEF FPC}UTF8Copy{$ELSE}Copy{$ENDIF}(s, LineBegin, n - LineBegin + 1));
end;
...
begin
  ...
  begin
    ...
    n :=  {$IFDEF FPC}UTF8Length{$ELSE}Length{$ENDIF}(AnsiStr);// Length(AnsiStr)
    Is2ByteCodepage := (n > Length(s));
    {$IFDEF FPC}
    Is2ByteCodepage := False;
    {$ENDIF}
