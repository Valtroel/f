...
begin
  ...
  if (Obj is TfrxView) then
    begin
      ...
      begin
        if v.Name <> '_pagebackground' then
          begin
            z := Round(v.AbsLeft * FDiv);
            if z < FMinX then
              FMinX := z;
            z := FYOffset + Round(v.AbsTop * FDiv);
            if z < FMinY then
              FMinY := z;
            z := Round((v.AbsLeft + v.Width) * FDiv) + 1;
            if z > FMaxX then
              FMaxX := z;
            z := FYOffset + Round((v.AbsTop + v.Height) * FDiv) + 1;
            if z > FMaxY then
              FMaxY := z;
          end;
        v.Draw(FBitmap.Canvas, FDiv, FDiv, 0, FYOffset);
      end;
    end;
end;
...
procedure TfrxIMGExportDialog.FormShow(Sender: TObject);
begin
