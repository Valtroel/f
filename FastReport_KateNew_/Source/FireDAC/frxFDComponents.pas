          ...
          if ADataSet.Report <> nil then
            ...
            begin
              if oFDParam.DataType in [ftString, ftWideString, ftFixedWideChar, ftWideMemo, ftFmtMemo, ftMemo] then
                oFrxParam.Value := ''
              else if oFDParam.DataType in [ftVariant, ftInteger, ftWord, ftLargeint, ftTimeStamp, ftFMTBcd, ftOraTimeStamp, ftLongWord, ftShortint, ftByte, ftExtended, ftSingle] then
                oFrxParam.Value := 0
              else
...
{-------------------------------------------------------------------------------}
initialization
  frxObjects.RegisterObject1(TfrxFDDataBase, nil, '', {$IFDEF DB_CAT}'DATABASES'{$ELSE}''{$ENDIF}, 0, 67);
  frxObjects.RegisterObject1(TfrxFDTable, nil, '', {$IFDEF DB_CAT}'QUERIES'{$ELSE}''{$ENDIF}, 0, 68);
  frxObjects.RegisterObject1(TfrxFDQuery, nil, '', {$IFDEF DB_CAT}'QUERIES'{$ELSE}''{$ENDIF}, 0, 69);
  frxObjects.RegisterObject1(TfrxFDStoredProc, nil, '', {$IFDEF DB_CAT}'QUERIES'{$ELSE}''{$ENDIF}, 0, 70);
finalization
