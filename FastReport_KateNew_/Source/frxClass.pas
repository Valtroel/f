...
{$ENDIF}
  ...
  public
    ...
    procedure ExportObject(Obj: TfrxComponent); virtual;
...
const
  DefFontName = {$IFDEF NONWINFPC}'Nimbus Sans L'{$ELSE}'Arial'{$ENDIF};
  DefFontSize = 10;
...
{$ENDIF}
    ...
    begin
      s := Text;
      SetLength(s, Length(s) - {$IFDEF FPC}Length(sLineBreak){$ELSE}2{$ENDIF});
{$IFDEF delphi12}
...
procedure TfrxCustomExportFilter.ExportObject(Obj: TfrxComponent);
//var v: TfrxView;
begin
  {if Obj is TfrxView then
    begin
      v := TfrxView(Obj);
      if not ExportNotPrintable and not (vsPrint in v.Visibility) then 
        v.Visibility := v.Visibility - [vsExport]; 
      //if ExportNotPrintable and not (vsPrint in v.Visibility) then
      //  v.Visibility := v.Visibility + [vsExport];
    end;}
end;
