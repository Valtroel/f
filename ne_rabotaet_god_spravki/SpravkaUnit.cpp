...
void __fastcall TSpravkaForm::Dop_text () {
   ...
   if (CheckBox2 -> Checked) // Обучается за счет бюджета или по договору
    { // if Обучается за счет бюджета или по договору
        ...
        else if(FDQuery1N_CK -> Value >0)
            Memo1 -> Lines -> Add
                ("          Обучается по договору о подготовке специалиста с высшим образованием за счет средств республиканского бюджета на условиях целевой подготовки."
                );
        else
            Memo1 -> Lines -> Add
