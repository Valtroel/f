    ...
    )
{
    ...
    else                                                                        // else реальная работа
    {
        ...
        // DataModule1 -> Database1 -> AliasName = "Dekanat_vmain1";               // обратиться к новому серверу main2
...
void __fastcall TSplashForm::FormActivate (TObject * Sender)
{
    ...
    try
    {
        LogPassForm = new TLogPassForm (Application);                           // Получение логина и пароля

        TIniFile * inif =                                                       // При наличии раздела в ini считываем в нём
            new TIniFile (
                ChangeFileExt (
                    Application -> ExeName,
                    ".INI"
        ...
        Application ->                                                          // Создаем форму  TDataModule1 для подключения к БД
            CreateForm (
        ...
        DataModule1 -> FDPhysFBDriverLink1 -> VendorLib =                       // Создаем форму  TDM2 для подключения к IBDatabase1
            AnsiString (
        ...
        if (    loginUser == "1"                                                // Передаем набранные в LogPassForm имя пользователя и пароль (пароли для DEVELOPER)
             || loginUser == "2"
             || loginUser == "3"
             || loginUser == "4"
             || loginUser == "5"
        ...
        if (DataModule1 -> FDConnection1 -> Connected)                          // проверка коннекта с БД, если конект прошел
        {
            ...
            if (DataModule1 -> IP2FDQ -> RecordCount)                           // пользователь найден
            {
                ...
                if (loginUser == "DEVELOPER")                                   // определяем глобальную переменную nom_fac (логин и пароль) (используется в MyDialogPass1)
                    nom_fac = fac_nomer;
                else
                    nom_fac =
                        DM2 -> FDTable1 ->
